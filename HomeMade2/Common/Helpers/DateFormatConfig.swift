...
extension Date {
    ...
    static func showDate(from date : String) ->String{
        let dateformetter = DateFormatter()
        dateformetter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
        dateformetter.timeZone = (NSTimeZone(name: "UTC")! as TimeZone)
        guard let date = dateformetter.date(from: date)
            else {
            return "--"
        }
        ...
        dateformetter2.dateFormat = "dd MMM yyyy"
        
        let dateString = dateformetter2.string(from: date)
        return dateString
    }
    
    
    static func DateAgo(from date : String) ->String{
        let dateformetter = DateFormatter()
        dateformetter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
        dateformetter.timeZone = (NSTimeZone(name: "UTC")! as TimeZone)
        guard let date = dateformetter.date(from: date)
        ...
        let currentDate = Date()
        let diffInTime = Calendar.current.dateComponents([.second,.minute,.hour,.day], from: date, to: currentDate)
        return stringFromCalender(dateComponent: diffInTime)
...
//        return dateString
    ...
    static func stringFromCalender(dateComponent : DateComponents) -> String{
        var string = ""
        
        let days = dateComponent.day ?? 0
        let hour = dateComponent.hour ?? 0
        let minutes = dateComponent.minute ?? 0
        let sec = dateComponent.second ?? 0
        ...
        if days > 0{
            ...
            if minutes > 1 {
                ...
                return "\(minutes) minute ago".localized
            }
        }
        else if sec > 0 {
            return "moments ago"
    ...
    static func makeServerDate(from date : Date) ->String{
        let dateformetter = DateFormatter()
        dateformetter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
        dateformetter.timeZone = (NSTimeZone(name: "UTC")! as TimeZone)
        let dateString = dateformetter.string(from: date)
        return dateString
    }
