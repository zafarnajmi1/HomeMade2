...
class OrderManager: RequestManager {

//    func updateUserAddresses( params: [String: Any] , completion:@escaping (Response<UserLoginAPIResponse,String>) -> Void) {
    func updateUserAddresses( params: [String: Any] , completionHandler: @escaping (LoginAPIResponse?, String?) -> Void){

//        let params = parameters.dictionary

        print("Parameters :\(params)")
        let url = UPDATE_USER_ADDRESS

        print("Update User Address Cart:\(url)")

        self.request(url, method: .post, parameters: params) { (response) in
            print("Update User Address Response :\(response)")
            ...
            if let error = self.checkError(Response: response){
                ...
                do {
                    ...
                    print("Update User Address Failed :\(error)")
...
//        let path = basePath + order.authOrdersUsers
    ...
    func fetchOrderStatus(completionHandler: @escaping (OrderStatusAPIResponse?, String?) -> Void){
        ...
        self.request(url, method: .get, parameters: nil) { (response) in
            ...
            if let error = self.checkError(Response: response){
                ...
                do {
                    let apiResponse = try JSONDecoder().decode(OrderStatusAPIResponse.self, from: response.data!)
                    completionHandler(apiResponse, nil)
                }
                catch let error {
                    print("ORDER STATUS Failed :\(error)")
                    let error = "Something went wrong, please try again later".localized
                    completionHandler(nil, error)
                }
    ...
    func completeProduct( params: [String: Any] , completionHandler: @escaping (APIResponse?, String?) -> Void){
    ...
    func shipProduct( params: [String: Any] , completionHandler: @escaping (APIResponse?, String?) -> Void){
        ...
        print("Ship Product URL:\(url)")
        
        self.request(url, method: .post, parameters: params) { (response) in
            print("Ship Product Response :\(response)")
            ...
            if let error = self.checkError(Response: response){
                ...
                do {
                    ...
                    print("Ship Product Failed :\(error)")
                    let error = self.parsingError
                    completionHandler(nil, error)
                }
    ...
    func cancelProduct( params: [String: Any] , completionHandler: @escaping (APIResponse?, String?) -> Void){
        
        //        let params = parameters.dictionary
        
        print("Parameters :\(params)")
        let url = CANCEL_PRODUCT
        
        print("Cancel Product URL:\(url)")
        ...
        self.request(url, method: .post, parameters: params) { (response) in
            ...
            if let error = self.checkError(Response: response){
                ...
                do {
                    ...
                    print("Cancel Product Failed :\(error)")
                    let error = self.parsingError
                    completionHandler(nil, error)
                }
